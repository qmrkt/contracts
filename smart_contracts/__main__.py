...
def _ensure_package_dir(path: Path) -> None:
    """Ensure generated artifact directories remain importable as Python packages."""
...
def _get_output_path(
    ...
    """Construct the generated client path for the contract folder name."""
...
def _compiler_env() -> dict[str, str]:
    """Build an environment that keeps the repo venv discoverable for PuyaPy."""
    env = os.environ.copy()
...
def build(output_dir: Path, contract_path: Path, contract_name: str) -> Path:
    """
    ...
    if output_dir.exists():
        rmtree(output_dir)
    _ensure_package_dir(output_dir.parent)
    _ensure_package_dir(output_dir)
    ...
    # Look for arc56.json files and normalize the generated client name to match
    # the long-standing repo layout consumed by tests and downstream packages.
    ...
    if not app_spec_file_names:
        ...
        for file_name in app_spec_file_names:
            ...
            generated_clients = list(output_dir.glob("client_*.py"))
            if not generated_clients and client_output_path.exists():
                generated_clients = [client_output_path]
            if not generated_clients:
                generated_clients = list(output_dir.glob("*_client.py"))
            if not generated_clients:
                raise Exception("Could not build contract client: PuyaPy did not emit a client file")
            if len(generated_clients) != 1:
                raise Exception(
                    f"Could not build contract client: expected exactly one generated client, got {len(generated_clients)}"
                )
            generated_client = generated_clients[0]
            if generated_client.resolve() != client_output_path.resolve():
                move(str(generated_client), str(client_output_path))
    if client_file:
    ...
    match action:
        case "build":
            for contract in filtered_contracts:
                ...
                build(artifact_path / contract.name, contract.path, contract.name)
        case "deploy":
            for contract in filtered_contracts:
